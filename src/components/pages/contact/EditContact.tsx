...
import { updateCont } from '../../store/contactReducer';
...
const EditContact = () => {
    ...
    const currentRow = contacts?.data?.items?.filter((item:any, index: number ) => index === userID);

    const [formData, setFormData] = useState < IFormInput > ({firstName: currentRow[0]?.firstName, lastName: currentRow[0]?.lastName, status: currentRow[0]?.status});
    ...
    const submitForm = (e : any) => {
        e.preventDefault();
       const finalArray =  contacts?.data?.items?.map((item:any, index:number)=>{
        ...
        dispatch(updateCont(finalArray));
        setstatus(true);
        setTimeout(() => {
            setstatus(false);
            navigate('/')
        ...
        <>
            ...
            <div className='w-full justify-center min-h-full'>
                <form method="POST"
                    ...
                    {
                    status && <p className='text-green-700 font-bold'>Contact updated successfully!</p>
                } </form>
            </div>
        </>
    )
